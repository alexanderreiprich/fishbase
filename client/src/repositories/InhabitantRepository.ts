...
export class InhabitantRepository {
  ...
  private async transformToInhabitant(
    ...
    const baseInhabitant = {
      ...
      image:
        ...
        "data" in apiData.image // Checks if this is a Buffer object that contains the image
          ? new Blob([new Uint8Array((apiData.image as any).data)], {
              type: "image/jpeg",
  ...
  private async parsePredators(predatorsString: string): Promise<Animal[]> {
    ...
    return results.filter(
      (predator): predator is Animal =>
        predator !== null &&
        (predator.type === InhabitantType.FISH ||
          predator.type === InhabitantType.INVERTEBRATE)
    )
  }
}
