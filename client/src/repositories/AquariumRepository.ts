...
export class AquariumRepository {
  ...
  public static getInstance(): AquariumRepository {
	...
	public async addInhabitantToAquarium(aquariumId: number, inhabitantId: number, amount: number): Promise<void> {
		try {
			const response = await fetch(
				`${this.baseUrl}/add`,
				{
			...
			console.error("Fehler beim Hinzufügen von Inhabitants in das Aquariums", error)
      throw error;
		}
	}
	...
	private async parseInhabitants(inhabitantString: string): Promise<Inhabitant[]> {
		if (!inhabitantString || inhabitantString.trim() === '') {
			return []
		}

		const inhabitantIds = inhabitantString.split(',').map(id => id.trim()).filter(id => id !== '')
		...
		const inhabitantPromises = inhabitantIds.map(async (id) => {
			try {
				...
				console.error(`Fehler beim Abrufen des Inhabitants mit ID ${id}:`, error)
				return null
			}
		})

		const results = await Promise.all(inhabitantPromises)
		return results.filter((inhabitant): inhabitant is Inhabitant => inhabitant !== null)
	}
}